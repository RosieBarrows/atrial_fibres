...
FOLDER="/data/Dropbox/Segmentations/2016111001EP/final_heart/LA_endo_fch/epi"
endo_um="LA_endo_um" 
cemrg_epi="/data/Dropbox/Segmentations/2016111001EP/final_heart/LA_endo_fch/epi/LA_epi_cemrg"

echo "converting endo vtk to carp"
cmd="meshtool convert -imsh=${FOLDER}/${endo_um} -ifmt=vtk -omsh=${FOLDER}/${endo_um} -ofmt=carp_txt"
eval $cmd 

echo "converting cemrg vtk to carp"
cmd="meshtool convert -imsh=${cemrg_epi} -ifmt=vtk -omsh=${cemrg_epi} -ofmt=carp_txt"
eval $cmd 
...
cmd="meshtool insert meshdata -imsh=${cemrg_epi} -msh=${FOLDER}/${endo_um}_shift -op=1 -outmsh=${FOLDER}/${endo_um}_shift_fibres"
eval $cmd 
